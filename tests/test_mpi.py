...
context = Context.Builder('mpi').build()
...
if context.comm.pid() == 0:
    ...
    server_comm.add_pipe(pipes.ToModel(lambda: LogisticRegression(28 * 28, 10)))
    ...
    server_comm.add_pipe(pipes.Infer(model_pos=0, criterion=nn.CrossEntropyLoss()))
    # --> global_model, training_size, round_id
    ...
    # --> client_id, model_states, sample_size, round_id
    client_comm.add_pipe(pipes.ToModel(lambda: LogisticRegression(28 * 28, 10)))
    # --> client_id, lr_model
    client_comm.add_pipe(pipes.TrainModel(
        epochs=10, optimizer_method='sgd', data_streamer=mnist_streamer,
        criterion=nn.CrossEntropyLoss()
    ))
    # --> client_id, trained_lr_model_states,sample_size
    client_comm.add_pipe(pipes.Infer(model_pos=1, criterion=nn.CrossEntropyLoss()))
    # --> client_id, trained_lr_model_states,sample_size
    client_comm.add_pipe(pipes.ToServer())
    # --> client_id, trained_lr_model_states,sample_size
    client_comm.run(context=context)
